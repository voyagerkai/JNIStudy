...
jint Java_com_example_myfristjnitest_MainActivity_arrayChannl(JNIEnv *env,
	...
	jclass cs = (*env)->FindClass(env,
			"com/example/myfristjnitest/MainActivity");
	if (cs == 0) {
		LOGI("c10");
	} else {
		LOGI("c11");
	}
	jmethodID md = (*env)->GetMethodID(env, cs, "getDog","()Lcom/example/myfristjnitest/Dog;");
	if (md == 0) {
		LOGI("m10");
	} else {
		LOGI("m11");
	}
	jobject obj = (*env)->CallObjectMethod(env, thiz, md);
...
jstring Java_com_example_myfristjnitest_Dog_LastTest(JNIEnv *env, jobject obj) {
	...
	jmethodID mid = (*env)->GetMethodID(env, cls, "helloTotal",
			"(ILjava/lang/String;)Ljava/lang/String;");
	if (mid == 0) {
		LOGI("m0");
	} else {
	...
	jstring str = (*env)->NewStringUTF(env, "likai");
	jstring res = (*env)->CallObjectMethod(env, obj, mid, 66, str);
